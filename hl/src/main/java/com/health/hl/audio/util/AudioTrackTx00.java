...
public class AudioTrackTx00 {
	...
	public EncoderTx00 encoderTx;

	/***********************************
	 * 构造函数，初始化硬件，建立audiotrack对象，建立解码类Encoder对象
	 */
	public AudioTrackTx00() {
	 ...
	 * @return 返回发送是否成功的标志
	 */
	public boolean msg_byte(int msg, int length) {
		encoderTx = new EncoderTx00(length);
		int sendedsize = 0;
		short[] msg_PCM;
		if (issending) {
			msgStop();
		...
		if (audioTrack.getState() != AudioTrack.STATE_UNINITIALIZED) {
			...
			msg_PCM = encoderTx.updateAudioTxBuf((short) msg);
